...
#      evaluation_agent/, analysis_agent/, report_engine/, llm/ (the set
...
for pkg in control_plane candidate_agent evaluation_agent analysis_agent report_engine llm; do
...
echo "build-artifacts: done. A NEW instance pulls this on its first boot."
echo "An already-running instance needs bootstrap re-run AND a service restart"
echo "(a reboot alone redeploys nothing — cloud-init's runcmd is per-instance,"
echo "and bootstrap's 'systemctl enable --now' does not restart running units):"
echo ""
echo "  aws ssm send-command --instance-ids <id> --document-name AWS-RunShellScript \\"
echo "    --parameters 'commands=[\"bash /opt/${PROJECT}/bootstrap.sh && systemctl restart control-plane.service engined.service\"]'"
echo ""
echo "See infra/README.md — Redeploying."
